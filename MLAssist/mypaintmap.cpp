...
void MyPaintMap::paintEvent(QPaintEvent *event){
	...
	if (!image2.isNull())
	{
        image2 = image2.scaled(this->width() * currentStepScaleFactor * scaleFactor,
			this->height() * currentStepScaleFactor * scaleFactor,
			Qt::KeepAspectRatio,
			Qt::SmoothTransformation);
	}*/
  ...
  //      QRectF sourceRect(basex, basey, widgetSize.width(), widgetSize.height());
		...
		//double mapX = m_zoomPos.x() / widgetSize.width() * m_collision->xsize;    //界面坐标点转换为地图坐标点
  //      double mapY = m_zoomPos.y() / widgetSize.height() * m_collision->ysize;
  //      double basex = mapX * blockSize * m_scaleFactor - widgetSize.width() / 2;            //地图坐标起始点 原本绘制应该在0 0 现在绘制在此坐标
  //      double basey = mapY * blockSize * m_scaleFactor - widgetSize.height() / 2;
  //      qDebug() << m_zoomPos << widgetSize << m_collision->xsize << m_collision->ysize;
  //      qDebug() << mapX << mapY << basex << basey;
		//QRectF sourceRect(basex, basey, widgetSize.width(), widgetSize.height());
		//painter.drawImage(renderRect, image.scaled(image.width() * m_scaleFactor, image.height() * m_scaleFactor), sourceRect);
		//painter.drawImage(renderRect, image2.scaled(image2.width() * m_scaleFactor, image2.height() * m_scaleFactor), sourceRect);
  //      qDebug() << renderRect << sourceRect;
        if(m_collision)
        {
