...
void GetMapIndex(const Nan::FunctionCallbackInfo<v8::Value>& info)
{
	...
	if (!g_CGAInterface->GetMapIndex(index1, index2, index3, mapFileName))
	{
		Nan::ThrowError("RPC Invocation failed.");
		return;
	}
...
void WalkToWorker(uv_work_t* req)
{
	...
	std::string mapFileName;
	if (!g_CGAInterface->GetMapIndex(initmap_index1, initmap_index2, initmap_index3, mapFileName)) {
		data->m_reason = -1;
		return;
	}
	...
	while (1)
	{
		...
		if (worldStatus == 9 && gameStatus == 3 && sx == 0.0f && sy == 0.0f)
		{
			...
			if (!g_CGAInterface->GetMapIndex(curmap_index1, curmap_index2, curmap_index3, mapFileName)) {
				data->m_reason = -1;
				return;
			}
