...
LUAPLUS_INLINE int LuaState::Resume(lua_State *from, int nargs, int* nres) {
#if LUA_VERSION_NUM == 501
	return lua_resume(LuaState_to_lua_State(this), nargs);
#elif LUA_VERSION_NUM == 502
	return lua_resume(LuaState_to_lua_State(this), from, nargs, int* nres);
#elif LUA_VERSION_NUM >= 504
	return lua_resume(LuaState_to_lua_State(this), from, nargs, nres);
#endif
}


LUAPLUS_INLINE int LuaState::Resume(LuaState *from, int nargs, int* nres) {
#if LUA_VERSION_NUM == 501
	return lua_resume(LuaState_to_lua_State(this), nargs);
#elif LUA_VERSION_NUM == 502
	return lua_resume(LuaState_to_lua_State(this), LuaState_to_lua_State(from), nargs);
#elif LUA_VERSION_NUM >= 504
	return lua_resume(LuaState_to_lua_State(this), LuaState_to_lua_State(from), nargs, nres);
#endif
}
...
LUAPLUS_INLINE int LuaState::LoadStringData(const char* str)
{
//	return luaL_loadbuffer(LuaState_to_lua_State(this), str, strlen(str), str);
	return luaL_loadstring(LuaState_to_lua_State(this), str);
}
