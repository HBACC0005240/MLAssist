...
namespace CGAServiceProtocol
{
	...
	TIMAX_DEFINE_PROTOCOL(GetCardsRecvMsg, cga_cards_recv_msg_t());
	...
	TIMAX_DEFINE_PROTOCOL(GetMapIndex, std::tuple<int, int, int, std::string>());
	...
	TIMAX_DEFINE_PROTOCOL(GetMouseXY, std::tuple<int, int>());
	TIMAX_DEFINE_PROTOCOL(GetMouseOrientation, int());
	...
	TIMAX_DEFINE_PROTOCOL(BackSelectServer, void());
	...
	TIMAX_DEFINE_PROTOCOL(AddAllTradeItems, void(int));
...
namespace CGA
{
	...
	public:
		...
		virtual bool GetCardsRecvMsg(cga_cards_recv_msg_t& info) {
			if (m_connected) {
				try {
		...
		virtual bool GetMapIndex(int& index1, int& index2, int& index3, std::string& filemap) {
			if (m_connected) {
				try {
					std::tuple<int, int, int, std::string> tup = m_client.call(std::chrono::milliseconds(10000), m_endpoint, CGAServiceProtocol::GetMapIndex);
					index1 = std::get<0>(tup);
					index2 = std::get<1>(tup);
					index3 = std::get<2>(tup);
					filemap = std::get<3>(tup);
					return true;
				}
				catch (timax::rpc::exception const& e) { if (e.get_error_code() != timax::rpc::error_code::TIMEOUT) m_connected = false; OutputDebugStringA("rpc exception from " __FUNCTION__); OutputDebugStringA(e.get_error_message().c_str()); }
				catch (msgpack::parse_error& e) { OutputDebugStringA("parse exception from " __FUNCTION__); OutputDebugStringA(e.what()); }
		...
		virtual bool GetMouseXY(int& x, int& y) {
			if (m_connected) {
				try {
					std::tuple<int, int> tup = m_client.call(std::chrono::milliseconds(10000), m_endpoint, CGAServiceProtocol::GetMouseXY);
					x = std::get<0>(tup);
					y = std::get<1>(tup);
					return true;
				}
		...
		virtual bool GetMouseOrientation(int& ndir) {
			if (m_connected) {
				try {
					ndir = m_client.call(std::chrono::milliseconds(10000), m_endpoint, CGAServiceProtocol::GetMouseOrientation);
					return true;
				}
				catch (timax::rpc::exception const& e) { if (e.get_error_code() != timax::rpc::error_code::TIMEOUT) m_connected = false; OutputDebugStringA("rpc exception from " __FUNCTION__); OutputDebugStringA(e.get_error_message().c_str()); }
				catch (msgpack::parse_error& e) { OutputDebugStringA("parse exception from " __FUNCTION__); OutputDebugStringA(e.what()); }
		...
		virtual bool CGAInterface::BackSelectServer()
		{
			if (m_connected) {
				try {
					m_client.call(std::chrono::milliseconds(10000), m_endpoint, CGAServiceProtocol::BackSelectServer);
					return true;
				}
				catch (timax::rpc::exception const& e) { if (e.get_error_code() != timax::rpc::error_code::TIMEOUT) m_connected = false; OutputDebugStringA("rpc exception from " __FUNCTION__); OutputDebugStringA(e.get_error_message().c_str()); }
				catch (msgpack::parse_error& e) { OutputDebugStringA("parse exception from " __FUNCTION__); OutputDebugStringA(e.what()); }
		...
		virtual bool SayWords(const std::string& str, int color, int range, int size) {
			if (m_connected) {
				try {
		...
		virtual bool ChangePetName(int petId, const std::string& name, bool& result) {
			if (m_connected) {
				try {
		...
		virtual bool PlayerMenuSelect(int menuindex, const std::string& menustring, bool& result) {
			if (m_connected) {
				try {
		...
		virtual bool AddAllTradeItems(int step) {
			if (m_connected) {
				try {
					m_client.call(std::chrono::milliseconds(10000), m_endpoint, CGAServiceProtocol::AddAllTradeItems, step);
					return true;
				}
				catch (timax::rpc::exception const& e) { if (e.get_error_code() != timax::rpc::error_code::TIMEOUT) m_connected = false; OutputDebugStringA("rpc exception from " __FUNCTION__); OutputDebugStringA(e.get_error_message().c_str()); }
				catch (msgpack::parse_error& e) { OutputDebugStringA("parse exception from " __FUNCTION__); OutputDebugStringA(e.what()); }
		...
		virtual bool RegisterBattleActionNotify(const std::function<void(int)>& callback)
		{
			...
			return false;
		}virtual bool RegisterGameWndKeyDownNotify(const std::function<void(unsigned int)>& callback)
		{
			if (m_connected)
			{
				try
				{
					m_async_client.sub(m_endpoint, CGAServiceProtocol::NotifyGameWndKeyDown,
						[callback](unsigned int flags) {
						if (callback)
							callback(flags);
					},
						[](auto const& e) {
