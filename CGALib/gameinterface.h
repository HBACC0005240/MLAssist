...
namespace CGA
{
	...
	typedef struct cga_player_info_s
	{
		...
		cga_player_info_s(int a1, int a2, int a3, int a4, int a5, int a6, int a7, int a8, int a9, int a10,
			...
			bool a25, bool a26,
			std::string& a27, std::string& a28, std::string& a29, const cga_playerpet_detail_info_t& a30, std::string& a31,int a32)
			:
			...
			name(a27), job(a28), nick(a29), detail(a30), gid(a31),player_index(a32)
		{
	...
	typedef struct cga_item_info_s
	{
		...
		cga_item_info_s(std::string& a1, std::string& a2, std::string& a3, int a4, int a5, int a6, int a7, int a8, bool a9, short a10, int a11)
			: name(a1), attr(a2), info(a3), itemid(a4), count(a5), pos(a6), level(a7), type(a8), assessed(a9), assess_flags(a10), image_id(a11)
		{
		}
		std::string name;
		std::string attr;
	...
	typedef struct cga_battle_unit_s
	{
		...
		cga_battle_unit_s(std::string& a1, int a2, int a3, int a4, int a5, int a6, int a7, int a8, int a9, int a10) :
			name(a1), modelid(a2), level(a3), curhp(a4), maxhp(a5), curmp(a6), maxmp(a7), flags(a8), pos(a9), petriding_modelid(a10)
		{
	...
	typedef struct Card_Recv_Msg
	{
		...
		Card_Recv_Msg(int a1, std::string& a2, std::string& a3)
	...
	typedef struct cga_card_recv_msg_s
	{
		cga_card_recv_msg_s()
		{
			index = 0;
		}
		cga_card_recv_msg_s(int a1, std::string& a2)
			: index(a1), name(a2)
		{
		}
		int index;
		std::string name;
		tCard_Recv_Msg msgs[10];
	}cga_card_recv_msg_t;

	typedef std::vector<cga_card_recv_msg_t> cga_cards_recv_msg_t;
...
#define UI_DIALOG_BATTLE_SKILL 2
	...
	public:
		...
		virtual bool GetMapIndex(int& index1, int& index2, int& index3, std::string& filemap) = 0;
		...
		virtual bool GetMouseXY(int& x, int& y) = 0;
		virtual bool GetMouseOrientation(int& ndir) = 0;
		...
		virtual bool BackSelectServer() = 0;
		virtual bool SayWords(const std::string& str, int, int, int) = 0;
		...
		virtual bool ChangePetName(int petId, const std::string& name, bool& result) = 0;
		...
		virtual bool PlayerMenuSelect(int menuindex, const std::string& menustring, bool& result) = 0;
		...
		virtual bool AddAllTradeItems(int step) = 0;
