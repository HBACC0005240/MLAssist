...
#define PET_STATE_WALK 16
	...
	typedef struct pet_s
	{
		...
		int skillslots;//6A4
		short unk1;	//6A8
		short battle_flags;	//6AA, 1 = ′y?ü 2=???· 3=xiuxi
		char realname[17];//??áúòá 6AC
		char name[19];
		int unk2;
		int walk;
	}pet_t;
...
#pragma pack()
	...
	public:
		...
		virtual int GetMouseOrientation(void);
		virtual std::tuple<int, int> GetMouseXY();
		virtual std::tuple<int, int, int, std::string> GetMapIndex();
	...
	public:
		...
		void WM_GetCardsRecvMsg(cga_cards_recv_msg_t* info);
		...
		void WM_GameWndKeyDown(unsigned int val);
...
#define WM_CGA_BACK_SELECT_SERVER WM_USER+11000
#define WM_CGA_GET_CARDS_RECV_MSG WM_USER+11001
