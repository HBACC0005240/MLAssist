...
void CGA_NotifyGameWndKeyDown(unsigned int key)
{
	if (server && server.get()) {
		server->pub("NotifyGameWndKeyDown", key);
	}
}
	...
	for (uint16_t port = CGA_PORT_BASE; port < CGA_PORT_BASE + 1000; port++)
	{
		...
		if (CGA_CreatePortMutex(port))
		{
			...
			try
			{
				...
				server->register_handler("GetMouseXY", timax::bind(&CGAService::GetMouseXY, &g_CGAService));
				server->register_handler("GetMouseOrientation", timax::bind(&CGAService::GetMouseOrientation, &g_CGAService));
				...
				server->register_handler("BackSelectServer", timax::bind(&CGAService::BackSelectServer, &g_CGAService));
				...
				server->register_handler("AddAllTradeItems", timax::bind(&CGAService::AddAllTradeItems, &g_CGAService));
