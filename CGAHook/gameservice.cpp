...
void CGAService::NewBATTLE_PlayerAction()
{
	...
	if (prevPlayerStatus == 2 && playerStatus == 0)//prevPlayerStatus==4二动
	{
		//Double action
		m_btl_double_action = true;
	}
	if (prevPlayerStatus != 1 && playerStatus == 1)	//playerStatus == 1人物回合  
	{
	...
	else if (prevPlayerStatus != 4 && playerStatus == 4)//playerStatus == 4宠物回合
	{
...
#define CONVERT_GAMEVAR(type, offset) (type)((ULONG_PTR)hGameBase + offset)
	...
	{
		m_hFont = CreateFontW(14, 0, 0, 0, FW_THIN, false, false, false,
			CHINESEBIG5_CHARSET, OUT_CHARACTER_PRECIS,
			CLIP_CHARACTER_PRECIS, DEFAULT_QUALITY,
			FF_MODERN, L"宋体");
			...
			if (S_OK == pSurface->GetDC(&hDC))
			{
				...
				for (int i = 0; i < 20; ++i)
				{
					if (m_battle_units[i].exist)
					{
						...
						SetTextColor(hDC, RGB(0, 0, 0));
						TextOutA(hDC, x - width / 2, y + 30, buf, len);
						SetTextColor(hDC, RGB(255, 255, 0));
						TextOutA(hDC, x - width / 2 - 1, y - 1 + 30, buf, len);
						if (i >= 10)
						{
							nUnitCount++;
							nLvSum += m_battle_units[i].level;
						}
					}
				}
				if (nLvSum != 0 && nUnitCount != 0)
				{
					sprintf(buf, "AvgLv:%d", nLvSum / nUnitCount);
					int len = strlen(buf);
					...
					TextOutA(hDC, 320 - width / 2, 20, buf, len);
				}
...
void CGAService::ParseBattleUnits(const char* buf)
{
	...
	while (1)
	{
		...
		m_battle_units[pos].petriding_modelid = NET_ParseHexInteger(buf, '|', position + 10);
...
cga_cards_recv_msg_t CGAService::GetCardsRecvMsg()
{
	cga_cards_recv_msg_t info;
	...
	for (int itempos = 0; itempos < maxitems; ++itempos)
	{
		if (IsItemValid(itempos))
		{
			...
			info->emplace_back(
				...
				(*g_playerBase)->iteminfos[itempos].image_id
			);
		}
	}
}
...
std::tuple<int, int, int, std::string> CGAService::GetMapIndex()
{
	char mapname[1024];
	format_mapname(mapname, *g_map_index1, *g_map_index2, *g_map_index3);
	return std::tuple<int, int, int, std::string>(*g_map_index1, *g_map_index2, *g_map_index3, mapname);
}
...
void CGAService::WM_GetBattleUnit(int pos, cga_battle_unit_t* u)
{
	...
	if (IsBattleUnitValid(pos))
	{
		...
		u->petriding_modelid = m_battle_units[pos].petriding_modelid;
	}
}
	...
	for (int i = 0; i < 20; ++i)
	{
		if (IsBattleUnitValid(i))
		{
			u->emplace_back(
				...
				m_battle_units[i].petriding_modelid
			);
		}
	}
}
...
void CGAService::AddAllTradeItems(int step)
{
...
{
	auto textutf8 = boost::locale::conv::from_utf<char>(menustring, "GBK");
	return SendMessageA(g_MainHwnd, WM_CGA_PLAYER_MENU_SELECT, (WPARAM)menuindex, (LPARAM)textutf8.c_str()) ? true : false;
}
...
void CGAService::BackSelectServer()
{
	SendMessage(g_MainHwnd, WM_CGA_BACK_SELECT_SERVER, 0, 0);
}

void CGAService::WM_BackSelectServer()
{
	if (*g_world_status_cgitem == 3 && *g_game_status_cgitem == 11)
	{
		*g_world_status_cgitem = 2;
		*g_game_status_cgitem = 1;

	}
}

void CGAService::WM_GameWndKeyDown(unsigned int val)
{
	CGA_NotifyGameWndKeyDown(val);
}

void CGAService::WM_GetCardsRecvMsg(cga_cards_recv_msg_t* info)
{
	if (!IsInGame())
		return;
