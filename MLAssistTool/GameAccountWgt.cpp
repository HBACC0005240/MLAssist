...
GameAccountWgt::GameAccountWgt(QWidget* parent)
	: QWidget(parent)
{
	ui.setupUi(this);
	init();
...
GameAccountWgt::~GameAccountWgt()
{
}

void GameAccountWgt::init()
{
	m_pAccountTreeModel = new AccountTreeModel(this);
	ui.treeView->setModel(NULL);
	ui.treeView->setModel(m_pAccountTreeModel);
...
void GameAccountWgt::resetModel()
{
	if (m_pAccountTreeModel)
	{
		delete m_pAccountTreeModel;
		m_pAccountTreeModel = nullptr;
	}
	m_pAccountTreeModel = new AccountTreeModel(this);
	ui.treeView->setModel(NULL);
	ui.treeView->setModel(m_pAccountTreeModel);
...
QString GameAccountWgt::DisplayInputDlg(const QString& title, const QString& labelName)
{
...
void GameAccountWgt::on_treeView_customContextMenuRequested(const QPoint& pos)
{
...
void GameAccountWgt::doAddAccountAssemble()
{
	QDialog dlg(this);
	dlg.setWindowFlags(dlg.windowFlags() & ~Qt::WindowContextHelpButtonHint);
...
void GameAccountWgt::doAddAccount()
{
	AddAccountDlg dlg;
...
void GameAccountWgt::doAddAccountGid()
{
