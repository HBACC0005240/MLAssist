...
void AccountTreeModel::SetupModelData()
{
	auto pObjList = ITObjectDataMgr::getInstance().GetDstObjTypeList(TObject_AccountIdentity);
	for (ITObjectPtr tObj : pObjList)
	{
		...
		auto pAccountAsse = qSharedPointerDynamicCast<ITAccountIdentity>(tObj);
...
void AccountTreeModel::SetupModelSubData(ITObjectPtr pObj, TreeItem* pItem)
{
	if (!pObj || !pItem)
		return;
...
bool AccountTreeModel::dropMimeData(const QMimeData* data, Qt::DropAction action, int row, int column, const QModelIndex& parent)
{
...
QMimeData* AccountTreeModel::mimeData(const QModelIndexList& indexes) const		//点击的Qmodel节点
{
